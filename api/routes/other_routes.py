...
streaming_route = APIRouter(prefix="/stream", tags=["Streaming"])
...
class StreamingTaskRequest(BaseModel):
    task_type: str = Field(..., description="Task type (e.g., resume_coach, learning)")
    payload: dict = Field(..., description="Task payload")
    session_id: Optional[str] = Field(None, description="Optional session ID for tracking")


async def stream_task_response(task_type: str, payload: dict):
    """
    Stream task response as server-sent events (SSE).
    Yields: data: <token>\n\n
    """
    from config import settings
    from core.task_router import router
    from core.streaming_manager import streaming_manager, stream_with_sinks

    if not settings.ENABLE_STREAMING:
        yield f"data: {json.dumps({'error': 'Streaming is disabled'})}\n\n"
        return

    try:
        session_id = payload.get("session_id", f"stream-{task_type}")
        session = streaming_manager.create_session(session_id, task_type)

        # For streaming, we use the LLM directly with streaming
        # This is a simple implementation that streams LLM output
        prompt_text = json.dumps(payload.get("payload", payload), indent=2)
        system_prompt = f"You are helping with {task_type} task. Respond concisely."
        ...
        token_count = 0
        async for token in llm_service.stream_async(prompt_text, system_prompt):
            token_count += 1
            yield f"data: {json.dumps({'type': 'token', 'data': token})}\n\n"

        # Signal completion
        yield f"data: {json.dumps({'type': 'done', 'data': {'tokens': token_count, 'session_id': session_id}})}\n\n"
        await session.emit_done({"tokens": token_count})
...
@streaming_route.post(
    "/task",
    summary="Stream task response as server-sent events",
    description="Returns task response tokens as they are generated by the LLM.",
)
async def stream_task(request: StreamingTaskRequest):
    """Stream a task response using server-sent events."""
    return StreamingResponse(
        stream_task_response(request.task_type, request.payload),
